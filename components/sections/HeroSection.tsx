...
export default function HeroSection() {
  ...
  return (
    ...
    >
      ...
      {/* Decorative plants */}
      <motion.div 
        className="absolute -bottom-20 -left-10 w-48 h-48 opacity-70 -z-10"
      ...
      <motion.div 
        className="absolute top-10 right-0 w-40 h-40 opacity-60 -z-10"
        initial={{ opacity: 0, y: -50 }}
        animate={{ opacity: loaded ? 0.6 : 0, y: loaded ? 0 : -50 }}
        transition={{ duration: 1, delay: 0.3 }}
      >
        <Image 
          src="/icons/botanical-leaf.svg" 
          alt="Decorative leaf" 
          fill
          className="object-contain"
          priority
      ...
      <motion.div 
        className="absolute top-1/4 left-1/3 w-20 h-20 opacity-70 -z-10"
        style={{ y }}
      ...
      <motion.div 
        className="absolute bottom-1/3 right-1/4 w-16 h-16 opacity-60 -z-10"
        style={{ y: useTransform(scrollYProgress, [0, 1], ["0%", "30%"]) }}
        initial={{ opacity: 0, rotate: 30 }}
        animate={{ opacity: loaded ? 0.6 : 0, rotate: 0 }}
      ...
      <motion.div 
        className="absolute top-2/3 left-1/4 w-14 h-14 opacity-50 -z-10"
        style={{ y: useTransform(scrollYProgress, [0, 1], ["0%", "50%"]) }}
        initial={{ opacity: 0, rotate: -20 }}
        animate={{ opacity: loaded ? 0.5 : 0, rotate: 0 }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                ...
                >
                  <Link 
                    ...
                    className={cn(
                      ...
                      "rounded-lg px-8 py-4 transition-all transform hover:scale-105 flex items-center justify-center",
                  ...
                  >
                    ...
                    <span className="absolute top-0 left-0 w-full h-full bg-[url('/icons/leaf.svg')] bg-no-repeat bg-right-top opacity-10" />
                  </Link>
                  
                  <Link 
                    href="#course" 
                    className="border border-primary/30 bg-white/80 backdrop-blur-sm text-primary hover:bg-primary/5 font-medium rounded-lg px-8 py-4 transition-all flex items-center justify-center shadow-md"
                  >
                    View Curriculum
                  </Link>
                </motion.div>
          ...
          >
            <div className="relative w-full h-[500px] rounded-[2.5rem] overflow-hidden">
              ...
              {/* Decorative borders */}
              <div className="absolute inset-0 border-[8px] border-white rounded-[2.5rem] opacity-40 pointer-events-none z-20" />
              <div className="absolute inset-0 border border-primary/20 rounded-[2.5rem] opacity-80 pointer-events-none z-20" />
            </div>
            
            {/* Instructor badge */}
            <motion.div 
              className="absolute -bottom-6 -right-6 bg-white shadow-xl rounded-2xl p-4 z-30 border border-gray-100"
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-gray-800 text-lg">Olivia</p>
